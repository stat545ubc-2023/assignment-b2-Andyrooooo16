...
#'
#' @description Given a data frame `data` and a column `group`, #'`count_all_missing_by_group()` creates a new data frame with #'one row per level of `group`. The first column of the output #'data frame contains the levels of `group`, and the rest of the #'columns contain the number of missing values for all columns in #'`data` except `group`.
#'
...
#' @param .groups (OPTIONAL) Controls whether the output data #'frame is grouped. By default, drops all levels of grouping #'(`"drop"`). Specify `"keep"` to keep the output data frame #'grouped by `group`. See `summarize()` documentation in `dplyr` #'for a full list of options.
#'
#' @return A data frame or tibble with the levels of `group`, and #'the number of NAs within each level of `group` for all columns #'in `data` except `group`.
#'
#' @export
#'
#' @examples
...
#' @details Example tests for count_all_missing_by_group function.
#'
#' @export
#'
#' @examples
